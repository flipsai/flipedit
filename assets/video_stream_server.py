...
class VideoStreamServer:
    def __init__(
        self, video_path_1, video_path_2, host, port, target_fps, jpeg_quality
    ):
        ...
        self._pause_lock = (
            threading.Lock()
        )  # Lock for thread-safe access to pause state
        ...
        logger.info(
            f"Initialized server with videos: {video_path_1} and {video_path_2}"
        )
    ...
    async def handle_client(self, websocket):
        ...
        logger.info(
            f"Client {client_info} added. Total clients: {len(self.connected_clients)}"
        )
        try:
            ...
            async for message in websocket:
                ...
                if message == "pause":
                    ...
                    logger.info("Received shutdown command. Stopping server.")
                    # Stop the asyncio event loop to trigger shutdown
            ...
            logger.info(
                f"Client {client_info} removed. Total clients: {len(self.connected_clients)}"
            )
    ...
    def _process_frames_thread(self):
        ...
        if not cap1.isOpened() or not cap2.isOpened():
            logger.error(
                "Error opening one or both video sources in processing thread."
            )
            if cap1.isOpened():
                cap1.release()
            if cap2.isOpened():
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if not ret1:
                    ...
                    ret1, frame1 = cap1.read()
                    if not ret1:
                        continue  # Skip if reading fails after reset
                if not ret2:
                ...
                frame1_resized = cv2.resize(
                    frame1,
                    (target_width_1, target_height),
                    interpolation=cv2.INTER_AREA,
                )
                frame2_resized = cv2.resize(
                    frame2,
                    (target_width_2, target_height),
                    interpolation=cv2.INTER_AREA,
                )
                ...
                try:
                    self._frame_queue.put(
                        frame_data, timeout=1
                    )  # Add timeout to prevent indefinite block
                except queue.Full:
        ...
        while not self._stop_event.is_set():
            ...
            if frame_count % 100 == 0:
                ...
                if total_elapsed > 0:
                    ...
                    logger.info(
                        f"Sent {frame_count} frames, current FPS: {actual_fps:.2f}, Queue size: {self._frame_queue.qsize()}"
                    )
                else:
                    logger.info(
                        f"Sent {frame_count} frames, Queue size: {self._frame_queue.qsize()}"
                    )
    ...
    async def start_server(self):
        ...
        self._processing_thread = threading.Thread(
            target=self._process_frames_thread, daemon=True
        )
