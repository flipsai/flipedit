...
class ComfyUIService {
  ...
  final ValueNotifier<String?> comfyUIPathNotifier = ValueNotifier<String?>(
    null,
  );
  ...
  final ValueNotifier<String> serverUrlNotifier = ValueNotifier<String>(
    'http://127.0.0.1:8188',
  );
  ...
  Future<bool> startComfyUI() async {
    ...
    try {
      ...
      _comfyProcess = await Process.start(pythonPath, [
        mainPyPath,
        '--listen',
        '0.0.0.0',
        '--port',
  ...
  Future<Map<String, dynamic>?> executeWorkflow(Workflow workflow) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final result = await _waitForWorkflowCompletion(
        ...
        status =
            'Error queuing workflow: ${response.statusCode} - ${response.body}';
        return null;
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>?> _waitForWorkflowCompletion(
    String promptId,
  ) async {
    try {
      ...
      while (!isComplete && attempts < maxAttempts) {
        ...
        if (response.statusCode == 200) {
          ...
          final isRunning = data['running'].any(
            (item) => item['prompt_id'] == promptId,
          );
          final isPending = data['pending'].any(
            (item) => item['prompt_id'] == promptId,
          );
