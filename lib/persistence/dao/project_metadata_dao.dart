...
class ProjectMetadataDao extends DatabaseAccessor<ProjectMetadataDatabase> with _$ProjectMetadataDaoMixin {
  ...
  Future<int> insertProjectMetadata(ProjectMetadataTableCompanion projectMetadata) {
  ...
  Future<ProjectMetadata?> getProjectMetadataById(int id) {
    return (select(projectMetadataTable)..where((t) => t.id.equals(id))).getSingleOrNull();
  }

  // Update a project metadata
  Future<bool> updateProjectMetadata(ProjectMetadataTableCompanion projectMetadata) {
