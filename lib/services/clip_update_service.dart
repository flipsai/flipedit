...
class ClipUpdateService {
  ...
  void updateClipFromCommand(Map<String, dynamic> state) async {
    ...
    if (state.containsKey('startTimeOnTrackMs')) {
      updatedClip = updatedClip.copyWith(
        startTimeOnTrackMs: state['startTimeOnTrackMs'],
      );
    }

    if (state.containsKey('endTimeOnTrackMs')) {
      updatedClip = updatedClip.copyWith(
        endTimeOnTrackMs: state['endTimeOnTrackMs'],
      );
    }

    if (state.containsKey('startTimeInSourceMs')) {
      updatedClip = updatedClip.copyWith(
        startTimeInSourceMs: state['startTimeInSourceMs'],
      );
    }

    if (state.containsKey('endTimeInSourceMs')) {
      updatedClip = updatedClip.copyWith(
        endTimeInSourceMs: state['endTimeInSourceMs'],
      );
    }
  ...
  void moveClip({
    ...
    final command = ClipCommand(oldState, newState, updateClipFromCommand);
