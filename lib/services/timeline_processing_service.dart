...
class TimelineProcessingService {
  ...
  int timelineToSourceFrame(ClipModel clip, int timelineFrame, int fps) {
    ...
    final clipStartFrameOnTimeline =
        (clip.startTimeOnTrackMs / 1000 * framerate).floor();
    ...
    final msInSource =
        ...
        (frameOffsetInClip / framerate * 1000).floor();
  ...
  bool isFrameInClip(ClipModel clip, int timelineFrame, int fps) {
    ...
    final clipDurationFrames =
        (getClipDurationMs(clip) / 1000 * framerate).ceil();
  ...
  Future<video_service.FrameInfo?> getFrameInfo(
    String videoPath,
    int frameIndex,
  ) async {
    try {
  ...
  video_service.FrameInfo getBlankFrame(int width, int height) {
    return video_service.FrameInfo(width: width, height: height);
  }
