  ...
  static Future<bool> checkTextureBridge() async {
    ...
    try {
      ...
      if (!await libraryFile.exists()) {
        logInfo(_logTag, 'Texture bridge library not found, extracting from assets...');
  ...
  static Future<void> _extractLibraryFromAssets(String targetDir) async {
    try {
      if (Platform.isMacOS) {
        ...
        final scriptData = await rootBundle.load('assets/build_texture_bridge.sh');
        ...
        final builtLibrary = File('${buildDir.path}/../bin/libtexture_bridge.dylib');
        if (await builtLibrary.exists()) {
          await builtLibrary.copy('$targetDir/libtexture_bridge.dylib');
          logInfo(_logTag, 'Texture bridge library copied to: $targetDir/libtexture_bridge.dylib');
        } else {
          ...
          if (await altLibrary.exists()) {
            ...
            logInfo(_logTag, 'Texture bridge library copied from alt path: $targetDir/libtexture_bridge.dylib');
          } else {
            logError(_logTag, 'Built library not found at: ${builtLibrary.path} or $altPath');
          }
        }
