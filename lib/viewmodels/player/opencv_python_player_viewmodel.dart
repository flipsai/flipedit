...
class OpenCvPythonPlayerViewModel extends ChangeNotifier implements Disposable {
  ...
  Future<void> _initializeAsync() async {
    try {
      ...
      try {
        ...
        logError(_logTag, 'Error getting UvManager from dependency injection: $e');
        if (!_isDisposed && statusNotifier.hasListeners) {
          statusNotifier.value = 'Error: Python integration service unavailable';
        }
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (!status['python_available']) {
          logError(_logTag, 'Python is not available: ${status['errors'].join(', ')}');
          if (!_isDisposed && statusNotifier.hasListeners) {
        ...
        if (!status['texture_bridge_available']) {
          logError(_logTag, 'Texture bridge is not available: ${status['errors'].join(', ')}');
          if (!_isDisposed && statusNotifier.hasListeners) {
        ...
        if (!status['websocket_responding']) {
          logError(_logTag, 'WebSocket server not responding: ${status['errors'].join(', ')}');
      ...
      if (testResult == -1) {
        logError(_logTag, 'Texture creation test failed - likely a plugin issue');
        if (!_isDisposed && statusNotifier.hasListeners) {
      ...
      logInfo(_logTag, 'Texture creation test succeeded, continuing with initialization');
      
      // Now try the normal flow through UvManager
      logInfo(_logTag, 'UvManager is available, initializing texture sharing...');
      ...
      logInfo(_logTag, 'initializeTextureSharing returned textureId: $textureId');
      
      if (textureId == -1) {
        logError(_logTag, 'Failed to initialize texture sharing, textureId is -1');
        if (!_isDisposed && statusNotifier.hasListeners) {
      ...
      logInfo(_logTag, 'OpenCvPythonPlayerViewModel initialized with textureId: $textureId');
    } catch (e, stackTrace) {
      logError(_logTag, 'Error initializing OpenCV Python player: $e', stackTrace);
      if (!_isDisposed && statusNotifier.hasListeners) {
  ...
  Future<void> _setupFrameDataListener() async {
    try {
      ...
      _frameDataSubscription = _frameDataChannel!.stream.listen(
        ...
        onDone: () {
          logWarning(_logTag, 'WebSocket connection closed, attempting to reconnect...');
          _reconnectFrameDataListener();
        },
      );
  ...
  void _handleFrameData(dynamic data) {
    try {
      ...
      if (command == 'frame_data') {
        ...
        logDebug(_logTag, 'Updated texture with frame $frameNumber (${width}x${height})');
      }
    } catch (e, stackTrace) {
  ...
  Future<void> _updateTexture(Uint8List frameData, int width, int height) async {
    try {
  ...
  void _updateFpsCounter() {
    ...
    if (now - _lastFpsUpdate >= 1000) { // Update every second
