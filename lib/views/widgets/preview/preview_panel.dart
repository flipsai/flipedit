        ...
        children: List.generate(videoUrls.length, (index) {
          ...
          return FutureBuilder<(VideoPlayerController, bool)>(
            ...
            builder: (context, snapshot) {
              ...
              print(
                "PreviewPanel FutureBuilder [$index: $videoUrl]: State=${snapshot.connectionState}",
              );
              if (snapshot.hasError) {
                print(
                  "PreviewPanel FutureBuilder [$index: $videoUrl]: ERROR=${snapshot.error}",
                );
              }
              if (snapshot.hasData) {
                ...
                print(
                  "PreviewPanel FutureBuilder [$index: $videoUrl]: Data received. Controller initialized: ${controller.value.isInitialized}",
                );
              }
              // --- DEBUG PRINTS END ---
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                // VideoPlayerWidget now gets controller via DI
                return VideoPlayerWidget(opacity: opacity);
              } else {
                // Should not happen
                print(
                  "PreviewPanel FutureBuilder [$index: $videoUrl]: Snapshot has no data and no error, connection state is ${snapshot.connectionState}. Showing fallback.",
                );
                return Center(
                  child: Icon(
                    Icons.question_mark,
