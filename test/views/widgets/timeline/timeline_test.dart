...
import 'package:flipedit/persistence/database/project_database.dart'
    as project_db; // Import for Track type
...
class MockUndoRedoService extends Mock
    implements UndoRedoService {} // Keep mock class definition

class MockValueNotifier<T> extends Mock
    implements ValueNotifier<T> {} // Add mock notifier helper
...
void main() {
  ...
  late ValueNotifier<List<ClipModel>>
  testClipsNotifier; // Needed for TimelineViewModel mock
  late ValueNotifier<int>
  testCurrentFrameNotifier; // Needed for TimelineNavigationViewModel mock
  ...
  setUp(() {
    ...
    when(
      mockTimelineViewModel.tracksNotifierForView,
    ).thenReturn(testTracksNotifier); // Stub this now

    // Stub TimelineNavigationViewModel properties
    when(
      mockTimelineNavigationViewModel.currentFrameNotifier,
    ).thenReturn(testCurrentFrameNotifier);
    when(
      mockTimelineNavigationViewModel.zoomNotifier,
    ).thenReturn(testZoomNotifier);
    when(
      mockTimelineNavigationViewModel.totalFramesNotifier,
    ).thenReturn(testTotalFramesNotifier);

    // Stub ProjectDatabaseService properties
    when(
      mockProjectDatabaseService.tracksNotifier,
    ).thenReturn(testTracksNotifier);
  ...
  group('Timeline Widget', () {
    testWidgets('should render Timeline widget correctly', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('should display timeline content based on view model', (
      WidgetTester tester,
    ) async {
